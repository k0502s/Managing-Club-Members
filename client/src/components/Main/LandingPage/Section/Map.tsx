...
const Map = () => {
    ...
    const onChangeSearchAdress = (e: React.ChangeEvent<HTMLInputElement>) => {
        e.preventDefault();
    ...
    const saveAdress = (e: React.ChangeEvent<HTMLInputElement>) => {
        e.preventDefault();
        if (Coords.length === 0) {
    ...
    const Maps = (address: string) => {
        ...
        // 주소로 좌표를 검색합니다
        geocoder.addressSearch(address, function (result: Array<any>, status: Array<string>) {
            // 정상적으로 검색이 완료됐으면
    ...
    return (
        <>
            ...
            <S.BtnhWrap>
                ...
                <S.Mapbtn color={'#54C5A0'} onClick={saveAdress}>
                    주소 설정
                </S.Mapbtn>
            </S.BtnhWrap>
        </>
