...
const LandingPage = () => {
    ...
    const { isAuthenticated, user, warnlisttoltal } = useSelector((state: State) => state.auth);
    const { chatalldata, membertoltal } = useSelector((state: State) => state.member);
    useEffect(() => {
    ...
    const authLink = (
        <>
            ...
            <hr />
            <Row>
                <S.HomeCard height={'400px'} margin={'0 15px 15px 15px'}>
                    <CardHeader>
                        <div>
                            <S.MapIcon />
                            SYSYTEM 기능 소개
                    ...
                    <CardBody>
                        ...
                        <CardText>
                            <S.HomeIcon />
                            <strong>HOME </strong>: 동호의 회원의 종류별 간단한 데이터를 확인 할 수 있으며 동호회 사이트의 다음 모임 장소의 지도 표시를 원격으로 지정할 수 있는 관리 기능이 있습니다.
                        </CardText>
                        <br />
                        <CardText>
                            <S.AddIcon />
                        ...
                        <CardText>
                            <S.WarnIcon />
                            <strong>WARN MEMBER LIST</strong>: MEMBER LIST에서 경고한 특정 회원 정보의 데이터를 담은 리스트를 보여주는 페이지 입니다. 여기서도 특정 회원을 바로 삭제(제명) 가능 할 수
                            있으며 경고를 해제할 수 있는 기능을 담고 있습니다.
                        </CardText>
                        <br />
                        <CardText>
